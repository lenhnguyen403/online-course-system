...
export default function TeacherCalendar() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader breadcrumbs={[{ to: '/teacher', label: 'Tổng quan' }, { label: 'Lịch' }]} title="Lịch dạy" description="Hạn bài tập và lịch thi theo lớp." />
            ...
            {events.map((ev) => (
              <li key={`${ev.type}-${ev.id}`} className="flex items-start gap-3 p-3 rounded-xl border border-slate-100 hover:bg-slate-50/50 transition-colors">
                ...
                {ev.type === 'assignment' && ev.classId && (
                  <Link to={`/teacher/classes/${ev.classId}/assignments/${ev.id}`} className="btn-secondary text-sm shrink-0">Xem bài nộp</Link>
                )}
              </li>
            ))}
          </ul>
